# 文档目录

## 文档索引

| 文档 | 说明 |
|------|------|
| [README.md](./README.md) | 项目说明、快速开始 |
| [OOP_STANDARD.md](./OOP_STANDARD.md) | OOP 编程规范（英文） |
| [ARCHITECTURE.md](./ARCHITECTURE.md) | 整体架构、设计原则 |
| [MODULES.md](./MODULES.md) | 模块职责、接口、依赖关系 |
| [TECH_REFERENCE.md](./TECH_REFERENCE.md) | Godot 4.3 GDScript 技术参考 |
| [BATTLE_FLOW_DESIGN.md](./BATTLE_FLOW_DESIGN.md) | 战斗流程设计 v2.0 |
| [RETROSPECTIVE.md](./RETROSPECTIVE.md) | 问题复盘、经验总结 |

---

## 快速查找

### 信号/事件问题
→ [TECH_REFERENCE.md](./TECH_REFERENCE.md#1-signal-vs-eventbus)

### 模块接口
→ [MODULES.md](./MODULES.md#模块职责表)

### 架构设计
→ [ARCHITECTURE.md](./ARCHITECTURE.md)

### 战斗流程
→ [BATTLE_FLOW_DESIGN.md](./BATTLE_FLOW_DESIGN.md)

---

## 文档更新记录

| 日期 | 版本 | 描述 |
|------|------|------|
| 2026-04-24 | v1.0 | 初始项目文档 |
| 2026-04-24 | v2.0 | 战斗流程系统设计 |
| 2026-04-24 | - | 模块化文档整理 |
