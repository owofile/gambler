## 着色器淡出销毁动画
class_name ShaderFadeDestroyAnimation
extends ShaderDestroyAnimation

var _material: ShaderMaterial = null

func _init():
	super._init("res://shaders/destroy/FadeDestroy.gdshader")
	_animation_name = "ShaderFadeDestroy"
...
func play(target: Node, config: Dictionary, on_complete: Callable) -> void:
	...
	set_target(target)
	_duration = config.get("duration", 0.4)
