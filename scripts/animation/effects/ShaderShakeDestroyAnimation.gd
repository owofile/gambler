## 着色器震动销毁动画
class_name ShaderShakeDestroyAnimation
extends ShaderDestroyAnimation

var _material: ShaderMaterial = null

func _init():
	super._init("res://shaders/destroy/ShakeDestroy.gdshader")
	_animation_name = "ShaderShakeDestroy"
...
func play(target: Node, config: Dictionary, on_complete: Callable) -> void:
	...
	set_target(target)
	_duration = config.get("duration", 0.5)
